...
cv2.imshow("raw img 1",img1)
cv2.imwrite("image1.png", img1)  # Change the filename and extension as needed
cv2.waitKey(0)
cv2.destroyAllWindows()

cv2.imshow("raw img 2",img2)
cv2.imwrite("image2.png", img2)  # Change the filename and extension as needed
cv2.waitKey(0)
cv2.destroyAllWindows()
...
cv2.imshow("uncropeed merged",leftAndMid)
cv2.waitKey(0)
cv2.destroyAllWindows()
...
panorama=img4[(width1//8):(4*width1//7),1:19*length1//20]#crop the picture because otherwise it will be too big for later process.


cv2.imshow("cropped",panorama)
cv2.imwrite("new.jpeg",panorama)
cv2.waitKey(0)
cv2.destroyAllWindows()
...
cv2.imshow("resizedd panorama",img4)
cv2.waitKey(0)
cv2.destroyAllWindows()
...
#    ret,img3=cap3.read()
    ...
    panorama=img4 #warpTwoImages(img4,img3,M2)

    panorama=panorama[(width1//8):(4*width1//7),1:19*length1//20]
    panorama=cv2.resize(panorama,(0,0),fx=0.4,fy=0.4)
    video.write(panorama)
