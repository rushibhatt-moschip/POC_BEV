from avinash_lib_panorama import Panaroma
import imutils
import cv2
...
for i in range(no_of_images):
    ...
    #images[i] = imutils.resize(images[i], width=2000)

for i in range(no_of_images):
    images[i] = imutils.resize(images[i], height=400)
    #images[i] = imutils.resize(images[i], height=2000)



#instance of class named panorama
panorama = Panaroma()
if no_of_images == 2:
